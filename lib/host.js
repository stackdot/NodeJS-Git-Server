...
if (fse.existsSync(repoLocation + '.db')) {
    fs.watch(repoLocation + '.db', { persistent: true }, function (event, fileName) {
      console.log('Event: ' + event)
      console.log(fileName + '\n')
    if (fse.existsSync(repoLocation + '.db')) {

      this.repos = fse.readJsonSync(repoLocation + '.db')
      if(this.repos.repos.length > 0 ){
        _this.bindEvents(function(){
        })
      }
      } else {
        this.repos = {repos: [], users: []}
      }
    })
  }
...
GitServer.prototype.bindEvents = function(callback) {
  var self = this;
  this.repos=fse.readJsonSync('/Users/lourdeslirosalinas/git-server/repos.db').repos
...
GitServer.prototype.createRepo = function(repo, callback) {
  ...
  if (this.getRepo(repo.name).name.length < 1) {
...
GitServer.prototype.onFetch = function(fetch) {
  ...
  if (this.getRepo(fetch.repo).name.length > 0) {
...
GitServer.prototype.onPush = function(push) {
  ...
  if (this.getRepo(push.repo).name.length > 0) {
...
GitServer.prototype.getRepo = function(repoName) {
  ...
  var emptyrepo = {name: [], anonRead: [], users: []}
  return emptyrepo;
};
