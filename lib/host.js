...
if (fs.existsSync('/Users/lourdeslirosalinas/git-server/repos.db')) {
    fs.watch('/Users/lourdeslirosalinas/git-server/repos.db', { persistent: true }, function (event, fileName) {
      console.log('Event: ' + event)
      console.log(fileName + '\n')
      //repositorios = JSON.parse(fs.readFileSync('/Users/lourdeslirosalinas/git-server/repos.db'))
//      this.repos = repositorios.repos;
    if (fs.existsSync('/Users/lourdeslirosalinas/git-server/repos.db')) {
      _this.bindEvents(function(){
        console.log('bindevents')
      })
      this.repos = JSON.parse(fs.readFileSync('/Users/lourdeslirosalinas/git-server/repos.db'))
...
//this.repos = JSON.parse(fs.readFileSync('/Users/lourdeslirosalinas/git-server/repos.db')).repos
      var i = this.repos.map(function(e) { return e.name; }).indexOf(repo.name);
      return this.checkTriggers(method, repo, gitObject);
    //  return this.checkTriggers(method, this.repos[i], gitObject);
    } else {
...
GitServer.prototype.onFetch = function(fetch) {
  ...
  console.log(repo.name + ' y este permiso: ' + repo.anonRead)
  if (repo !== false) {
...
GitServer.prototype.checkTriggers = function(method, repo, gitObject) {
  ...
  //this.repos[2].event(repo, gitObject);
...
GitServer.prototype.getRepo = function(repoName) {
  ...
  //_ref = this.repos;
  //_ref = JSON.parse(fs.readFileSync('/Users/lourdeslirosalinas/git-server/repos.db')).repos
/*  if (fs.existsSync('/Users/lourdeslirosalinas/git-server/repos.db')) {
    var repositorios = JSON.parse(fs.readFileSync('/Users/lourdeslirosalinas/git-server/repos.db'))
  } else {
    repositorios = {
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    repo = _ref[_i];
    console.log('nombre: '+ repo.name + '  y tipo ' + typeof[repo] + repo)
